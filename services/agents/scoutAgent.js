...
/**
 * Scout Agent - Live Team Stats + Qualifier Data
 * No Qwen call - pure data enrichment.
 */
...
// Cache qualifier stats after first load
...
function computeTournamentStats(teamName, allResults) {
  const teamResults = allResults.filter(function(r) { return r.team1 === teamName || r.team2 === teamName; });
  var goalsFor = 0, goalsAgainst = 0, formArr = [];
...
async function enrichTeamStats(team1, team2, baseStats) {
  ...
  console.log('[SCOUT] ' + team1 + ': ' + t1.tournamentPlayed + ' WC games | qualifier: ' + (q1 ? q1.won + 'W-' + q1.drawn + 'D-' + q1.lost + 'L' : 'no data'));
  console.log('[SCOUT] ' + team2 + ': ' + t2.tournamentPlayed + ' WC games | qualifier: ' + (q2 ? q2.won + 'W-' + q2.drawn + 'D-' + q2.lost + 'L' : 'no data'));
