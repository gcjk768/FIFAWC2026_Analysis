...
async function fetchEspnStats(team1, team2, kickoffSgt) {
  try {
    // ESPN buckets matches by US date — try the UTC date first, SGT date second
    const ko = new Date(kickoffSgt);
    const dateCandidates = [...new Set([
      ko.toISOString().slice(0, 10),
      ko.toLocaleDateString('en-CA', { timeZone: 'Asia/Singapore' }),
    ])].map((d) => d.replace(/-/g, ''));
    ...
    if (!event) {
      console.log(`[RESULTS] ESPN: no event found for ${team1} vs ${team2} (dates tried: ${dateCandidates.join(', ')})`);
      return null;
    }
